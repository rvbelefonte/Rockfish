...
Coordinate and elevation attributes are stored in SEG Y trace headers as signed
integers.  According to the SEG Y format, these are converted to real values by
applying scaling and unit codes that are stored in other header attributes.  To
access or edit real values for header coordinates and elevations, SEG Y files 
can be read with ``scale_headers=True``.

.. todo: Provide a scale_headers example.
...
``unpack_data=False``.  (Note that setting ``headonly=True`` will override the
``unpack_data`` option.)  The ``data`` class artibute will not show up in 
ipython's tab completion, but data are read directly from the disk when it is
accessed:
