...
def rayfan2db(rayfan_file, raydb_file=':memory:', noise=None,
    ...
    :param pickdb: Optional. An active 
        ...
        trace rays in the rayfan_file. Values for extra fields such as trace_in_file
        are copied from this database to the new pick database along with rayfan
        data. Default is ignore these extra fields.
    """
    ...
    for rfn in rays.rayfans: 
        for i,_t in enumerate(rfn.travel_times):
            ...
            if pickdb is not None:
                pick = pickdb.get_picks(event=[event], ensemble=[d['ensemble']],
                                        trace=[d['trace']])
                if len(pick) > 0:
