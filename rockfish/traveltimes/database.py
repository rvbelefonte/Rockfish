...
class DatabaseConnection(SQLiteConnection):
    ...
    def __init__(self, database, *args, **kwargs): 
        if database == ':memory:':
            logger.warn('Creating new database in memory.')
        ...
        if database == ':memory:':
            self.__file__ = ':memory:'
        else:
            self.__file__ = os.path.abspath(database)
    ...
    def execute(self, *args):
        ...
        Execute sql code
        """
        logger.debug('executing SQL: ' + ' '.join([str(a) for a in args]))
    ...
    def _get_primary_fields(self, table):
        ...
        logger.debug('found primary fields: ' + str(primary_fields))
    ...
    def _get_required_fields(self, table):
        ...
        logger.debug('found required fields: ' + str(required_fields))
    ...
    def _delete(self, table, **kwargs):
        ...
        sql += " WHERE " + ' and '.join(['{:}="{:}"'.format(k, kwargs[k])\
                                         for k in kwargs])
        logger.debug("calling: self.execute('%s')" % sql)
        self.execute(sql)
    ...
    def _insertupdate(self, table, **kwargs):
        ...
        logger.debug('found %s existing values', count)
...
class TraveltimeDatabaseConnection(DatabaseConnection):
    ...
    def _create_epsg_definitions(self, rebuild=False, view='epsg'):
        ...
        logger.info("(Re)creating view '{:}'".format(view))
    ...
    def _create_source_table(self, rebuild=False, table='source_pts'):
        ...
        logger.info("(Re)creating table '{:}'".format(table))
    ...
    def verify(self):
        ...
        for table in ['events', 'picks', 'rays', 'receiver_pts',
            ...
            if table not in self.TABLES:
                ...
                logger.info("Found required table '{:}'.".format(table))
    ...
    def _verify_epsg(self):
        ...
        if len(d) != 1:
            ...
            _d = d[0]
            logger.info('Primary projection is set to: {:} {:} {:}'\
    ...
    def _sort_pick_data(self, **kwargs):
        ...
        logger.debug('Sorted pick data as: {:}'.format(data))
    ...
    def insert_geom_from_SEGY(self, segy, model_line=None,
        ...
        for i, tr in enumerate(segy.traces):
            ...
            logger.info('ensemble=rid: {:}, trace=sid: {:}'\
                         .format(rid, sid))
