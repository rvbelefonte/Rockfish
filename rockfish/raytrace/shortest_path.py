"""
Wrappers for VM Tomography shortest path raytracing
"""
import os
import subprocess
import warnings
...
from rockfish.models import readVM
...
def raytrace_from_ascii(vmfile, rayfile, instfile='inst.dat',
    ...
    ninst = len(inst)
    logger.info('Raytracing paths to {:} receiver(s)...'.format(ninst))
    ...
    for i, _inst in enumerate(inst):
        ...
        # Build input
        logger.info(' Tracing rays for receiver #{:} ({:} of {:})'\
        ...
        if verbose or (stdout is not None):
        ...
        if (raysize1 == raysize0):
            logger.warn('Did not appear to trace rays for receiver #{:}'\
                        .format(_inst))

        logger.info('  Completed raytracing for receiver #{:} in {:} usec.'\
                    .format(_inst, elapsed))

    logger.info('Completed raytracing for all recievers in {:} usec.'\
                .format(time.clock() - start_all))

    if os.path.isfile(rayfile):
        logger.info('Output rayfile is: {:}'.format(rayfile))

    elif not os.path.isfile(rayfile):
        logger.warn('Did not create a rayfile.')
