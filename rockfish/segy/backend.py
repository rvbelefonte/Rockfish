...
from rockfish.segy.header import ENDIAN, DATA_SAMPLE_FORMAT_UNPACK_FUNCTIONS, \
...
class SEGYBinaryFileHeader(object):
    ...
    def write(self, file, endian=None):
        ...
        for item in BINARY_FILE_HEADER_FORMAT:
            length, name, _ = item
            logger.debug("Packing binary header %s=%s", name,
                          getattr(self, name))
            # Unpack according to different lengths.
            if length == 2:
                format = '%sh' % endian
...
class SEGYTrace(object):
    ...
    def _readTrace(self, unpack_headers=False, headonly=False,
        ...
        if scale_headers and not computed_headers:
            self.header = SEGYScaledTraceHeader(trace_header,
                                          endian=self.endian,
                                          unpack_headers=unpack_headers)
        elif computed_headers:
            self.header = SEGYComputedTraceHeader(trace_header,
                                          endian=self.endian,
                                          unpack_headers=unpack_headers)

        else:
            self.header = SEGYTraceHeader(trace_header, endian=self.endian,
                                          unpack_headers=unpack_headers)
        ...
        if headonly and not unpack_data:
            msg = """
                  Reading trace data with headonly=True overrides
                  unpack_data=False. No data will be loaded.
                  """.strip()
            warnings.warn(msg)
        if headonly:
            self.data = None
            ...
            if isinstance(self.file, StringIO.StringIO):
                msg = """
                      Attempted to read trace data from StringIO instance with
                      unpack_data=False.  The unpack_data option is only valid for
                      file pointers with a name class attribute.
                      """.strip()
                raise SEGYTraceReadingError(msg)
            # build a function for reading data from the disk on the fly
            self._unpack_data = OnTheFlyDataUnpacker(
                DATA_SAMPLE_FORMAT_UNPACK_FUNCTIONS[self.data_encoding],
                self.file.name, self.file.mode, pos, npts, endian=self.endian)
        if headonly or not unpack_data:
            # advance the file
    ...
    def __getattr__(self, name):
        ...
        if name == 'data':
            ...
            if hasattr(self, '_unpack_data'):
                return self._unpack_data()
            else:
                msg = """
                      Attempted to unpack trace data on the fly with
                      self._unpack_data(), but function does not exist.
                      """.strip()
...
class SEGYScaledTraceHeader(SEGYTraceHeader):
    """
    Extension of the :class:`SEGYTraceHeader` that handles scaling of
    ...
    def __init__(self, header=None, endian='>', unpack_headers=False):
        ...
        :param unpack_headers: Bool. Determines whether or not all headers will
            ...
            after being read. Defaults to False.
        """
        # create the standard header attributes
        SEGYTraceHeader.__init__(self, header=header, endian=endian,
                                 unpack_headers=unpack_headers)

    # Setters and getters for handling scaled header coordinates and elevations
    def _get_scaled_source_coordinate_x(self):
        return get_scaled_coordinate(self, 'source_coordinate_x')

    def _set_unscaled_source_coordinate_x(self, value):
        return set_unscaled_coordinate(self, 'source_coordinate_x', value)

    def _get_scaled_source_coordinate_y(self):
        return get_scaled_coordinate(self, 'source_coordinate_y')

    def _set_unscaled_source_coordinate_y(self, value):
        return set_unscaled_coordinate(self, 'source_coordinate_y', value)

    def _get_scaled_group_coordinate_x(self):
        return get_scaled_coordinate(self, 'group_coordinate_x')

    def _set_unscaled_group_coordinate_x(self, value):
        return set_unscaled_coordinate(self, 'group_coordinate_x', value)

    def _get_scaled_group_coordinate_y(self):
        return get_scaled_coordinate(self, 'group_coordinate_y')
    ...
    def _set_unscaled_receiver_group_elevation(self, value):
        return set_unscaled_elevation(self, 'receiver_group_elevation', value)

    def _get_scaled_surface_elevation_at_source(self):
    ...
    def _set_unscaled_datum_elevation_at_source(self, value):
        return set_unscaled_elevation(self, 'datum_elevation_at_source', value)

    def _get_scaled_water_depth_at_source(self):
        return get_scaled_elevation(self, 'water_depth_at_source')

    def _set_unscaled_water_depth_at_source(self, value):
        return set_unscaled_elevation(self, 'water_depth_at_source', value)

    # register the scaled properties
    logger.debug('Registering scaled properties.')
    ...
    scaled_surface_elevation_at_source = property(
        fget=_get_scaled_surface_elevation_at_source,
        fset=_set_unscaled_surface_elevation_at_source)
    scaled_source_depth_below_surface = property(
        fget=_get_scaled_source_depth_below_surface,
        fset=_set_unscaled_source_depth_below_surface)
    scaled_datum_elevation_at_receiver_group = property(
        fget=_get_scaled_datum_elevation_at_receiver_group,
        fset=_set_unscaled_datum_elevation_at_receiver_group)
    scaled_datum_elevation_at_source = property(
        fget=_get_scaled_datum_elevation_at_source,
        fset=_set_unscaled_datum_elevation_at_source)
    scaled_water_depth_at_source = property(
        fget=_get_scaled_water_depth_at_source,
        fset=_set_unscaled_water_depth_at_source)

class SEGYComputedTraceHeader(SEGYScaledTraceHeader):
    """
    Extension of :class:`SEGYTraceHeader` and  
    :class:`SEGYScaledTraceHeader`  that adds common values that calculated from
    other trace-header attributes.
    """
    ...
    def __init__(self, header=None, endian='>', unpack_headers=False,
        ...
        :param unpack_headers: Bool. Determines whether or not all headers will
            ...
            memory usage and the performance. They can be unpacked on-the-fly
            after being read. Defaults to False.
        :param ellps: String name of a valid :mod:`pyproj` ellipse for use in
            calculating distances and azimuths for geographic coordinates.
        """
        # create the standard header attributes
        SEGYScaledTraceHeader.__init__(self, header=header, endian=endian,
                                       unpack_headers=unpack_headers)

    def _calc_offset(self, ellps='WGS84'):
        """
        Computes source-receiver offset.  As a by-product, forward and back
        azimuths are also calculated.
        """
        if self.coordinate_units == 2:
            # Units are geographic
            gd = Geod(ellps=ellps)
            (faz, baz, dist_m) = gd.inv(self.scaled_source_coordinate_x,
                                        self.scaled_source_coordinate_y,
                                        self.scaled_group_coordinate_x,
                                        self.scaled_group_coordinate_y)
    ...
    def _get_source_receiver_offset(self):
        return self._calc_offset()[0]
    
    def _get_forward_azimuth(self):
        return self._calc_offset()[1]
    
    def _get_back_azimuth(self):
        return self._calc_offset()[2]
